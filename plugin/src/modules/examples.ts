...
export class BasicExampleFactory {
  ...
  static registerNotifier() {
    // 注册 Zotero.Notifier 观察者，监听 tab/item/file 事件并转发到 hooks.onNotify。
    // 同时在插件 shutdown 时自动解绑，避免重复注册和内存泄漏。
    const callback = {
      notify: async (
        event: string,
        type: string,
  ...
  static registerPrefs() {
    // 在 Zotero 设置中注册插件偏好页（preferences.xhtml）。
...
export class KeyExampleFactory {
  ...
  static registerShortcuts() {
    // 注册全局键盘监听：示例中将 Shift+L/Shift+S 转发给 hooks.onShortcuts。
...
export class UIExampleFactory {
  ...
  static registerStyleSheet(win: _ZoteroTypes.MainWindow) {
    ...
    // 示例：给 item pane 打一个 class，方便在 CSS 中做演示效果。
  ...
  static registerRightClickMenuPopup(win: Window) {
    // 在已注册菜单项前插入一个带子菜单的 popup 结构（menu -> menuitem）。
    // 这里演示了如何用 anchor 元素控制插入位置。
  ...
  static registerWindowMenuWithSeparator() {
    // 在 Zotero 顶部 File 菜单中插入分隔线 + 自定义菜单项。
  ...
  static async registerExtraColumn() {
    // 在条目列表注册一个简单文本列（由 dataProvider 返回字符串）。
    const field = "test1";
  ...
  static async registerExtraColumnWithCustomCell() {
    // 在条目列表注册一个“自定义单元格渲染”的列（renderCell 可返回自定义 DOM）。
    const field = "test2";
    await Zotero.ItemTreeManager.registerColumns({
      pluginID: addon.data.config.addonID,
      dataKey: field,
  ...
  static registerItemPaneCustomInfoRow() {
    // 在右侧 Item Pane 的条目信息区插入一行可编辑字段。
    // 这里演示将该行与条目 title 字段做双向读写。
  ...
  static registerItemPaneSection() {
    // 注册一个基础的「条目信息窗格(Item Pane)」区块（库视图右侧边栏）。
    // 只要选中条目，该区块就会参与渲染。
    Zotero.ItemPaneManager.registerSection({
      ...
      onRender: ({ body, item, editable, tabType }) => {
        // `body` 是区块内容容器；你可以在这里渲染任意 DOM/UI。
        // 该示例仅输出一段调试信息。
  ...
  static async registerReaderItemPaneSection() {
    // 注册一个「仅阅读器可用」的条目信息窗格区块。
    // 仅当当前 tabType === "reader"（PDF 阅读器）时才会启用。
    Zotero.ItemPaneManager.registerSection({
      ...
      // Optional
      // 可选：用一段 markup 预填充区块内容。
      // 注意：这里使用 XUL 的 <browser> 元素来嵌入内容页面。
      bodyXHTML:
...
export class PromptExampleFactory {
  ...
  static registerNormalCommandExample() {
    // 注册一个常规 Prompt 命令：Shift+P 打开后始终可见。
  ...
  static registerAnonymousCommandExample(window: Window) {
    // 注册一个匿名命令：根据输入执行条目检索，并渲染自定义建议列表。
    // 点击建议项后会切换到 Zotero 主 pane 并选中对应条目。
    ztoolkit.Prompt.register([
      {
        id: "search",
  ...
  static registerConditionalCommandExample() {
    // 注册一个带显示条件的命令：
    // when() 返回 true 才在 Prompt 列表中展示（这里要求当前有选中条目）。
    ztoolkit.Prompt.register([
      {
