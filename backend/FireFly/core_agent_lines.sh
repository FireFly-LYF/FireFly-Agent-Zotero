...
echo "firefly line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "firefly/agent")
core_bus=$(count_top_level_py_lines "firefly/bus")
core_config=$(count_top_level_py_lines "firefly/config")
core_cron=$(count_top_level_py_lines "firefly/cron")
core_heartbeat=$(count_top_level_py_lines "firefly/heartbeat")
core_session=$(count_top_level_py_lines "firefly/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "firefly/agent/tools")
extra_skills=$(count_skill_lines "firefly/skills")
extra_api=$(count_recursive_py_lines "firefly/api")
extra_cli=$(count_recursive_py_lines "firefly/cli")
extra_channels=$(count_recursive_py_lines "firefly/channels")
extra_utils=$(count_recursive_py_lines "firefly/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under firefly/agent"
echo "  - tools/ is counted separately from firefly/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, firefly.py, root files"
