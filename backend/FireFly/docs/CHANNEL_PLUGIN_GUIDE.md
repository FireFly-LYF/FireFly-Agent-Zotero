...
Build a custom firefly channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of firefly (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

firefly discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `firefly gateway` starts, it scans:

1. Built-in channels in `firefly/channels/`
2. External packages registered under the `firefly.channels` entry point group
...
```
firefly-channel-webhook/
├── firefly_channel_webhook/
...
```python
# firefly_channel_webhook/__init__.py
from firefly_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# firefly_channel_webhook/channel.py
import asyncio
...
from firefly.config.schema import Base
...
# pyproject.toml
[project]
name = "firefly-channel-webhook"
version = "0.1.0"
dependencies = ["firefly", "aiohttp"]

[project.entry-points."firefly.channels"]
webhook = "firefly_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
firefly plugins list      # verify "Webhook" shows as "plugin"
firefly onboard           # auto-adds default config for detected plugins
```

Edit `~/.firefly/config.json`:
...
```bash
firefly channels login <channel_name>
firefly channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `firefly onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `firefly.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `firefly.config.schema.Base`:
...
Override `default_config()` so `firefly onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `firefly-channel-{name}` | `firefly-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `firefly_channel_{name}` | `firefly_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/firefly-channel-webhook
cd firefly-channel-webhook
pip install -e .
firefly plugins list    # should show "Webhook" as "plugin"
firefly gateway         # test end-to-end
```

## Verify

```bash
$ firefly plugins list
