...
Use firefly programmatically — load config, run the agent, get results.
...
### `FireFly.from_config(config_path?, *, workspace?)`

Create a `FireFly` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.firefly/config.json`. |
...
```python
from firefly.agent import AgentHook, AgentHookContext
...
async def main():
    bot = FireFly.from_config(workspace="/my/project")
