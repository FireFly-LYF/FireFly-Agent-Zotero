"""JSON Schema 片段类型：均继承 :class:`~firefly.agent.tools.base.Schema`，用于描述与约束工具参数。

- ``to_json_schema()``: returns a dict compatible with :meth:`~firefly.agent.tools.base.Schema.validate_json_schema_value` /
  :class:`~firefly.agent.tools.base.Tool`.
...
Shared validation and fragment normalization are on the class methods of :class:`~firefly.agent.tools.base.Schema`.
...
from firefly.agent.tools.base import Schema


class StringSchema(Schema):
    """字符串参数：``description`` 用于字段说明；可选长度边界与枚举。"""
...
def tool_parameters_schema(
    ...
    **properties: Any,
    ...
    """为 :meth:`Tool.parameters` 构建根参数 ``{"type": "object", "properties": ...}``。"""
    return ObjectSchema(
        required=required,
