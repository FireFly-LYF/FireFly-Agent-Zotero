...
class SkillsLoader:
    ...
    def _parse_firefly_metadata(self, raw: str) -> dict:
        """从 frontmatter 解析技能元数据 JSON（支持 firefly 与 openclaw 键）。"""
        try:
            data = json.loads(raw)
        ...
        if not isinstance(data, dict):
            return {}
        payload = data.get("firefly", data.get("openclaw", {}))
    ...
    def _get_skill_meta(self, name: str) -> dict:
        """获取技能的 firefly 元数据（缓存于 frontmatter）。"""
        meta = self.get_skill_metadata(name) or {}
        return self._parse_firefly_metadata(meta.get("metadata", ""))
    ...
    def get_always_skills(self) -> list[str]:
        ...
        return [
            ...
            and (
                self._parse_firefly_metadata(meta.get("metadata", "")).get("always")
                or meta.get("always")
            )
        ]
