...
对于 `firefly`，自定义技能应位于活动工作区 `skills/` 目录下，以便可以在运行时自动发现它们（例如 `<workspace>/skills/my-skill/SKILL.md`）。
...
尽量减少前面的内容。在 `firefly` 中，需要时也支持 `metadata` 和 `always`，但除非实际需要，否则请避免添加额外的字段。
