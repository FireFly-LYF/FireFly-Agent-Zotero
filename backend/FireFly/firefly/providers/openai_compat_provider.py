...
from firefly.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from firefly.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from firefly.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/firefly",
    "X-OpenRouter-Title": "firefly",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply FireFly attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _is_reasoning_param_error(e: Exception) -> bool:
        """Return True when provider rejects thinking/reasoning parameters."""
        body = (
            getattr(e, "body", None)
            or getattr(e, "doc", None)
    ...
    @staticmethod
    def _strip_reasoning_kwargs(kwargs: dict[str, Any]) -> dict[str, Any]:
        """Drop optional thinking/reasoning params and keep request model-agnostic."""
        clean = dict(kwargs)
        clean.pop("reasoning_effort", None)
        extra = dict(clean.get("extra_body") or {})
        for key in ("enable_thinking", "thinking", "reasoning", "thinkingbudget", "thinking_budget"):
            extra.pop(key, None)
        if extra:
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("FIREFLY_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
