...
from firefly.agent.tools.web import WebSearchTool
from firefly.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="firefly", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("firefly.agent.tools.web.DDGS", MockDDGS, raising=False)
    import firefly.agent.tools.web as web_mod
