"""Tests for the FireFly programmatic facade."""
...
from firefly.firefly import FireFly, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        FireFly.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = FireFly.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("firefly.config.loader.load_config") as mock_load, \
         patch("firefly.firefly._make_provider") as mock_prov:
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from firefly.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = FireFly.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from firefly.config.schema import Config
    from firefly.firefly import _make_provider
...
def test_import_from_top_level():
    from firefly import FireFly as N, RunResult as R
    assert N is FireFly
