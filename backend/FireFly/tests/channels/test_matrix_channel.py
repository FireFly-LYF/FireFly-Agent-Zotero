...
from firefly.channels.matrix import _build_matrix_text_content

import firefly.channels.matrix as matrix_module
from firefly.bus.events import OutboundMessage
from firefly.bus.queue import MessageBus
from firefly.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from firefly.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("firefly.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "firefly.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("firefly.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "firefly.channels.matrix.asyncio.create_task", _fake_create_task
    )
