...
from firefly.providers.base import LLMResponse
...
def _make_loop():
    ...
    with patch("firefly.agent.loop.ContextBuilder"), \
         ...
         patch("firefly.agent.loop.SubagentManager"):
...
class TestRestartCommand:
    ...
    async def test_restart_sends_message_and_calls_execv(self):
        from firefly.command.builtin import cmd_restart
        from firefly.command.router import CommandContext
        ...
        with patch.dict(os.environ, {}, clear=False), \
             patch("firefly.command.builtin.os.execv") as mock_execv:
    ...
    async def test_restart_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch, \
             patch("firefly.command.builtin.os.execv"):
    ...
    async def test_status_intercepted_in_run_loop(self):
        ...
        with patch.object(loop, "_dispatch", new_callable=AsyncMock) as mock_dispatch:
            ...
            mock_dispatch.assert_not_called()
            out = await asyncio.wait_for(bus.consume_outbound(), timeout=1.0)
            assert "firefly" in out.content.lower() or "Model" in out.content
