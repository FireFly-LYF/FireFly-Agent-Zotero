"""Tests for tool hint formatting (firefly.utils.tool_hints)."""
...
class TestToolHintKnownTools:
    ...
    def test_read_file_long_path(self):
        result = _hint([_tc("read_file", {"path": "/home/user/.local/share/uv/tools/firefly/agent/loop.py"})])
    ...
    def test_exec_abbreviates_paths_in_command(self):
        ...
        cmd = "cd D:\\Documents\\GitHub\\firefly\\.worktree\\tomain\\firefly && git diff origin/main...pr-2706 --name-only 2>&1"
        result = _hint([_tc("exec", {"command": cmd})])
    ...
    def test_exec_abbreviates_linux_paths(self):
        ...
        cmd = "cd /home/user/projects/firefly/.worktree/tomain && make build"
        result = _hint([_tc("exec", {"command": cmd})])
        assert "\u2026/" in result
    ...
    def test_exec_abbreviates_home_paths(self):
        ...
        cmd = "cd ~/projects/firefly/workspace && pytest tests/"
        result = _hint([_tc("exec", {"command": cmd})])
        assert "\u2026/" in result
