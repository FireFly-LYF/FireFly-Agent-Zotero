...
def test_build_user_prompt_includes_query_and_tools():
    prompt = Summarizer.build_user_prompt(
        user_query="总结实验步骤",
...
def test_replace_last_assistant_text_skips_tool_call_rows():
    ...
    Summarizer.replace_last_assistant_text(messages, "用户可见总结")
    assert messages[-1]["content"] == "用户可见总结"
    assert messages[0]["content"] == "calling tool"


def test_summarize_reply_streams_and_returns_text():
    provider = MagicMock()
...
def test_collect_user_query_includes_injected_followups():
    query = Summarizer.collect_user_query(
...
def test_finalize_stream_reply_skips_summarizer_without_tools():
    provider = MagicMock()
    summarizer = Summarizer(provider)
    ...
    result = asyncio.run(
        summarizer.finalize_stream_reply(
            "test-model",
            user_query="问题",
            tool_events=[],
            agent_draft="直接回复",
...
def test_finalize_stream_reply_summarizes_once_after_tool_task():
    provider = MagicMock()
    summarizer = Summarizer(provider)
