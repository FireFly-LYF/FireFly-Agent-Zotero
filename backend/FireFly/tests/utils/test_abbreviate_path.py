...
import os
from firefly.utils.path import abbreviate_path
...
class TestAbbreviatePathWindows:
    def test_windows_drive_path(self):
        path = "D:\\Documents\\GitHub\\firefly\\src\\utils\\helpers.py"
        result = abbreviate_path(path, max_len=40)
        assert result.endswith("helpers.py")
        assert "firefly" in result

    def test_windows_home(self):
        home = os.path.expanduser("~")
        path = os.path.join(home, ".firefly", "workspace", "log.txt")
