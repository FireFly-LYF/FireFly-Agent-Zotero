...
def _literal_query_tokens(query: str) -> list[str]:
    """Split a non-regex query into OR tokens when ``|`` or CJK keywords are used."""
    stripped = query.strip()
    if "|" in stripped:
        return [part.strip() for part in stripped.split("|") if part.strip()]
    parts = stripped.split()
    if len(parts) >= 2 and all(_has_cjk(part) for part in parts):
        return parts
    return []


def _literal_match_info(query: str, text: str) -> tuple[bool, list[dict] | None]:
    """Case-insensitive literal match; multi-token queries use OR semantics."""
    text_lower = text.lower()
    query_lower = query.strip().lower()
    if query_lower and query_lower in text_lower:
        return True, None

    tokens = _literal_query_tokens(query)
    if not tokens:
        return False, None
        ...
        matched.append(
            {
                "start": start,
                "end": start + len(token),
                "match": text[start : start + len(token)],
                "token": token,
            }
        )
    if not matched:
...
class ReadingMixin:
    ...
    def search_text(self, query: str, *, regex: bool = False) -> list[dict]:
        """Search body, footnotes, and comments.

        When regex is false (default), query is a case-insensitive literal
        substring. ``|`` and whitespace-separated CJK keywords use OR semantics
        (any token may match). When regex is true, query is a Python regex.
        """
        ...
        for source, rel_path in targets:
            ...
            for para in tree.iter(f"{W}p"):
                ...
                if compiled is not None:
                    ...
                    ok, match_info = _literal_match_info(query, text)
                    if not ok:
                        continue
                results.append(
                    {
