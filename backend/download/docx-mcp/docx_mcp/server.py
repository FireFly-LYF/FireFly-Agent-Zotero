...
def search_text(query: str, regex: bool = False, document_handle: str = "") -> str:
    ...
    Args:
        query: Text to find. With regex=false (default), matched as a literal
            substring (case-insensitive). ``|`` or whitespace-separated CJK
            keywords use OR (any token may match). English multi-word phrases
            like ``"quick brown fox"`` still require the full phrase. With
            regex=true, query is a Python regex (``|`` means OR).
        regex: If true, treat query as a regex pattern.

    Returns matching paragraphs with paraId, source, and context. ``[]`` only
    means no match for this query — not an empty document; use
    ``get_headings()`` / ``get_document_info()`` to check structure.
    Prefer one anchor keyword per call (e.g. ``"步骤1"``) when verifying docx.
    """
    _, doc = _resolve(document_handle)
